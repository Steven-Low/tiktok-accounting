...
async function extractSellerDiscounts() {
  ...
  if (discountElement) {
    const discountText = discountElement.textContent.trim();  // Get the full text (e.g., "RM-28.3")
...
function extractSales() {
  ...
  if (saleElement) {
    const saleText = saleElement.textContent.trim();  // Get the full text (e.g., "RM100.0")
...
async function processAllPages() {
  ...
  const paginationItems = paginationList.querySelectorAll('.theme-arco-pagination-item[aria-label^="Page"]');
  ...
  async function processNextPage() {
    ...
    if (pageItem) {
      ...
      await timer(500); // Wait for 1 second
